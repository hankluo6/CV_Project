...
class PoissonSeamlessCloner:
    #@profile
    def __init__(self, mask, src_rgb, target_rgb, solver, scale):  
        self.mask = mask
...
if __name__ == "__main__":
    ...
    mask = utils.read_image(args.data_dir + "/mask.jpg", scale=args.scale, gray=True)
    src_rgb = utils.read_image(args.data_dir + "/source.jpg", scale=args.scale, gray=False)
    target_rgb = utils.read_image(args.data_dir + "/target.jpg", scale=args.scale,  gray=False)

    cloner = PoissonSeamlessCloner(mask, src_rgb, target_rgb, args.solver, args.scale)
